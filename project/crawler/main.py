...
ACCESS_KEY = "u2o5cMTKQIYQewqDFe/H52ovGJhz3KdGpP0ET70Zn1ky1ozreGJoBA6w30C6k8q+QPeJNEpKpmJmfrWWJKIFPg=="
ACCOUNT_NAME = "adgenstorage"
ENDPOINT = "https://adgenstorage.table.core.windows.net/advertisement"

CATEGORY_EN = ['womanclothe', 'manclothe', 'food', 'home_decoration', 'digital', 'office', 'living', 'beauty', 'sports', 'health', 'kitchen', 'pets', 'hobby', 'car', 'book', 'travel']
CATEGORY_KO = ['여성패션', '남성패션', '식품', '가구/홈인테리어', '가전/디지털', '문구/오피스', '생활용품', '뷰티', '스포츠/레저', '건강식품/헬스', '주방용품', '반려동물용품', '완구/취미', '자동차용품', '도서/CD/DVD', '여행']
...
def get_chrome_browser(service=None, options=None):
    ...
    if options is None:
        ...
        options.add_argument('user-agent=Mozilla/5.0 (Macintosh; Intel Mac OS X 10_12_6) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/97.0.1072.76 Safari/537.36')
...
if __name__ == '__main__':
    ...
    bestUnit = WebDriverWait(browser, 5).until(EC.presence_of_element_located((By.XPATH, '/html/body/div[3]/section[2]/div[9]')))
    time.sleep(2)
    ...
    for category in CATEGORY_EN:
        ...
        entity = TableEntity()
        bestCategory = WebDriverWait(browser, 10).until(EC.presence_of_element_located((By.XPATH, f"//*[@id=\"{prefix+category}\"]/dl/dd[3]/ul")))
        prod_list = bestCategory.find_elements(By.TAG_NAME, "li")
        random_num = random.randint(1, len(prod_list))
        item_info = WebDriverWait(browser, 5).until(EC.presence_of_element_located((By.XPATH, f"//*[@id=\"{prefix+category}\"]/dl/dd[3]/ul/li[{random_num}]/a"))) 
